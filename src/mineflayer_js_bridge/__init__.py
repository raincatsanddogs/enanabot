...
async def _(bot: Bot, event: Event, args: Message = CommandArg()):
    global js_process, active_bot, active_event, js_stop_requested, js_stderr_lines
...
async def listen_to_js():
    ...
    if js_process:
        await js_process.wait()
        return_code = js_process.returncode
        ...
        if (not js_stop_requested) and return_code not in (None, 0):
            ...
            if active_bot and active_event:
                try:
                    await active_bot.send(active_event, alert)
                except Exception as e:
                    logger.error(f"发送 JS 异常退出消息失败: {e}")
